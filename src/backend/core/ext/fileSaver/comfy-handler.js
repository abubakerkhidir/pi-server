...
export class ComfyFileSaver {
  ...
  extractFileInfo(toolName, result) {
    if (toolName && comfyTools.includes(toolName)) {
      ...
      trace(`[extractComfyFileInfo] Parsed result keys: ${Object.keys(parsed)}`);
      if (!parsed.asset_url) {
        trace(`[extractComfyFileInfo] No asset_url found in parsed result`);
        return null;
      }
      const fileInfo = {
      ...
      debug(`[extractComfyFileInfo] Extracted file info:`, JSON.stringify(fileInfo));
      return fileInfo;
    }
    return null
  }
  ...
  async processToolResult(fileInfo, downloadsDir) {
    if (fileInfo?.isComfyAsset) {
      debug(`[processComfyAsset] Downloading from: ${fileInfo.fileUrl}`);
  ...
  modifyToolResult(newUrl, result, fileId, filePath, fileInfo) {
    if (fileInfo?.isComfyAsset) {
      trace(`[modifyToolResult] Called with:`, { fileId, filePath, });
      ...
      if (modified.asset_url) {
        trace(`[modifyComfyToolResult] Replacing top-level asset_url: ${modified.asset_url} -> ${newUrl}`);
        modified.asset_url = newUrl;
      }
      if (modified.image_url) {
        trace(`[modifyComfyToolResult] Replacing top-level image_url: ${modified.image_url} -> ${newUrl}`);
        modified.image_url = newUrl;
      }

      // Handle MCP format: content array with JSON string in text block
      if (Array.isArray(modified.content)) {
        trace(`[modifyComfyToolResult] Modifying result.content array`);
      ...
      if (modified.details?.mcpResult?.content) {
        trace(`[modifyComfyToolResult] Modifying details.mcpResult.content array`);
        modified.details.mcpResult.content = modifyContentBlocks(modified.details.mcpResult.content, newUrl);
      }

      debug(`[modifyComfyToolResult] Modified result keys: ${Object.keys(modified)}`);
      return modified;
    }
    return undefined
  }
