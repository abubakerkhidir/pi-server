...
export class Html2PdfFileSaver {
  ...
  extractFileInfo(toolName, result) {
    if (toolName && tools.includes(toolName)) {
      trace(`[Html2PdfHandler:extractHtml2PdfFileInfo] Called with result type: ${typeof result}`);
      // Parse the MCP result format first
      const parsed = parseMcpResult(result);
      if (!parsed) {
        trace(`[Html2PdfHandler:extractHtml2PdfFileInfo] parseMcpResult returned null`);
        return null;
      }
      if (!parsed.outputPath) {
        trace(`[Html2PdfHandler:extractHtml2PdfFileInfo] No outputPath found`);
        return null;
      }
      ...
      debug(`[Html2PdfHandler:extractHtml2PdfFileInfo] Extracted file info:`, fileInfo);
      return fileInfo;
    }
    return null
  }
  ...
  processToolResult(fileInfo, downloadsDir) {
    if (fileInfo.isLocalFile) {
      trace(`[Html2PdfHandler:processLocalFile] Copying from: ${fileInfo.localPath}`);
      ...
      debug(`[Html2PdfHandler:processLocalFile] Copied to: ${filePath}, size: ${stats.size}, from: ${fileInfo.localPath}`);
  ...
  modifyToolResult(newUrl, result, fileId, filePath, fileInfo) {
    if (fileInfo.isLocalFile) {
      trace(`[Html2PdfHandler:modifyHtml2PdfToolResult] Called with:`, { fileId, filePath, serverBaseUrl, });
      // Start with a copy of the original result
      const modified = { ...result };
      modified.piFileId = fileId;
      if (modified.outputPath) {
        trace(`[Html2PdfHandler:modifyHtml2PdfToolResult] Replacing outputPath: ${modified.outputPath} -> ${newUrl}`);
      ...
      if (Array.isArray(modified.content)) {
        trace(`[Html2PdfHandler:modifyHtml2PdfToolResult] Modifying result.content array`);
      ...
      if (modified.details?.mcpResult?.content) {
        trace(`[Html2PdfHandler:modifyHtml2PdfToolResult] Modifying details.mcpResult.content array`);
        modified.details.mcpResult.content = modifyContentBlock(modified.details.mcpResult.content, newUrl, filePath);
      }
      debug(`[Html2PdfHandler:modifyHtml2PdfToolResult] Modified result keys: ${Object.keys(modified)}`);
      return modified;
    }
    return undefined
  }
