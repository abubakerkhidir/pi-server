...
export async function comfyViewImgExt(pi,event, ctx){
    ...
    if (event.toolName === "mcp" && event.input?.tool?.includes('view_image') && event.input.args) {
        trace('***** on mcp tool call for comfyViewImgExt: ', JSON.stringify(event))
        const args = JSON.parse(event.input.args)
        const assetId = args?.asset_id;
        if (!assetId) {
            debug(`[comfyViewImgExt] No asset_id in args, letting through`);
            return null;
        }
        ...
        if (f) {
            ...
            debug(`[comfyViewImgExt] Found local file, creating rejection: `,text);
            //      ctx.ui.notify(`Blocked unsafe command: ${command}`, "error");
            throw new Error(text);
        }
        debug(`[comfyViewImgExt] Asset not found locally, letting through`);
    }
}
