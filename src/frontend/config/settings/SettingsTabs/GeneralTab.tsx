  ...
  return (
    <>
      ...
      <div className="settings-section">
        ...
        {numField("sampling_temperature", "Sampling Temperature", "- How deterministic or creative the agent should be in response, range 0-2  (0: deterministic, 2: creative)", 0, 2)}
        {numField("sampling_top_p", "Sampling Top-P", "- % at which tokens should be selected, 0.9 means select samples with accumlative% > 90%.", 0.9, 1)}
...
function numFieldFun(settings: Settings, onChange: (field: keyof Settings, value: unknown) => void) {
  return (field: keyof Settings, label: string, desc: string, fallback: number, max: number) => (
    <div className="setting-item" key={String(field)}>
      <label>
        {label}
        <span style={{ fontSize: 11, color: "var(--text-muted)" }}>— {desc}</span>
